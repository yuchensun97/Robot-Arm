...
while ~isDone
   ...
   [qNext, isDone] = potentialFieldStep_22(qCurr, map, robot);
   % Take the step
   lynxServo(qNext);
   qCurr = qNext;
   [jointPos,~]=calculateFK_sol(qCurr,robot);
   scatter3(jointPos(7,1),jointPos(7,2),jointPos(7,3),'filled');
   % Pause for simulation
   % You may change this to speed up the sim or slow it down
end
